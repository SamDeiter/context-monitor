...
class ContextMonitor:
    ...
    def copy_handoff(self):
        ...
        tokens_left = max(0, context_window - tokens_used)
        percent = min(100, round((tokens_used / context_window) * 100))
        project_name = self.get_project_name(self.current_session['id'], skip_vscode=True)
        
        home = Path.home()
        brain_path = home / '.gemini' / 'antigravity' / 'brain' / self.current_session['id']
        handoff = f"""# CONTEXT BRIDGE: {project_name.upper()}
I am continuing work on the `{project_name}` project. We are reaching the context window limit ({percent}% used).

## 📍 Critical Paths
- **Conversation ID:** `{self.current_session['id']}`
- **Brain Folder:** `{brain_path}`
- **Task List:** `{brain_path}\\task.md`
- **Plan:** `{brain_path}\\implementation_plan.md`
- **Logs:** `{brain_path}\\.system_generated\\logs\\`

## 📊 Token Usage
- **Used:** {tokens_used:,} / {context_window:,}
- **Remaining:** {tokens_left:,}

## 🚀 Next Steps for Agent
1. Read `{brain_path}\\task.md` to see the current progress.
2. Read the latest log in `.system_generated\\logs\\` for the most recent context.
3. Continue the task as outlined in the implementation plan.
"""
